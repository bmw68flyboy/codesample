...
if($message_type != "RECURRING_INSTALLMENT_SUCCESS") { exit(); }
...
if($message_type == "RECURRING_INSTALLMENT_SUCCESS" && !$is_error) {
  $error[] = "No error!";
  ...
  $recur_support_query = mysql_fetch_assoc(mysql_query("SELECT * FROM products WHERE product_type='recur_support' ORDER BY product_id DESC LIMIT 1"));
  $total_recur_support_purchases = mysql_num_rows(mysql_query("SELECT NULL FROM user_products WHERE user_product_cancelled='0' AND user_product_gateway='2checkout' AND user_product_transID='{$sale_id}' AND user_product_product_id='{$recur_support_query['product_id']}'"));
  if($total_recur_support_purchases != 0) {
    if(time() > $user_info['user_date_supportexpires']) {
      $support_expires = ceil((time() + 2592000) / 86400) * 86400;
    } else {
      $support_expires = ceil(($user_info['user_date_supportexpires'] + 2592000) / 86400) * 86400;
    }
    mysql_query("UPDATE users SET user_date_supportexpires='{$support_expires}' WHERE user_id='{$user_info['user_id']}'"); 
    
    // ADD NOTE
      $note = "Ongoing support renewed: {$user_info['user_date_supportexpires']}, New support: {$support_expires}";
      mysql_query("INSERT INTO user_notes (user_note_user_id, user_note_text) VALUES ('{$user_info['user_id']}', '{$note}')");
  } 
  ...
  if($purchase != 0 || $cancelled != 0) {
      ...
      $date = date("F j, Y", $support_expires);
      $message = "Hello,\n\nThanks again for your ongoing support purchase. Your support has been extended and will renew again automatically on $date . To submit a support ticket login to the client area at: ".href('client-index')."\n\nPlease let us know if you have any questions or concerns!\n\nBest Regards,\nThe Team\nemail.net\n\n--------------------------------\nDO NOT REPLY TO THIS EMAIL - Your reply will not be received. If you require assistance, please contact us here: ".href('contact-page');
    ...
    // ADD NOTE
    $note = "Ongoing support renwed although support was cancelled: 2checkout $sale_id";
    mysql_query("INSERT INTO user_notes (user_note_user_id, user_note_text) VALUES ('{$user_info['user_id']}', '{$note}')");
