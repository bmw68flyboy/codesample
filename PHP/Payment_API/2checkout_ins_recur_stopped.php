...
if($message_type != "RECURRING_STOPPED") { exit(); }
...
if($message_type == "RECURRING_STOPPED" && !$is_error) {
  $error[] = "No error!";

      // SEND NOTIFICATION EMAIL TO USER
      $subject = "Ongoing Support Cancelled";
      $date = date("F j, Y", $user_info['user_date_supportexpires']);
      $message = "Hello,\n\nYour ongoing support has been cancelled. Please note: your current support will end $date unless renewed. If you have not requested this change please contact us. Please let us know if you have any questions or concerns!\n\nBest Regards,\nThe Team\nemail.net\n\n--------------------------------\nDO NOT REPLY TO THIS EMAIL - Your reply will not be received. If you require assistance, please contact us here: ".href('contact-page');
      $headers = "From: The Team <noreply@email.net>";
      mail('$user_info['user_email']', $subject, $message, $headers);
        
      // ADD NOTE
      $note = "Ongoing support stopped: 2checkout $sale_id";
